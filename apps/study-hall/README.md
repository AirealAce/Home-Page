...
- There is one visible card pane, fixed at 32rem high with no resize handle. Long content scrolls inside it. Reveal and rating controls stay below it in the same positions, with unavailable actions disabled. In mixed-audio decks, inactive audio controls are hidden while their space is reserved. List markers are added after flattening nested paragraph/div blocks, keeping each bullet or number beside its text and avoiding extra blank lines between items.
- Escape or Read formatted card pauses shortcuts and switches the same pane to the current side's formatting, images, and links. The text reader is hidden in this mode, and focus moves to the formatted content outside application mode. Resume keyboard study and Repeat current question or answer return to the text reader and announce its text again. Tab and Shift+Tab are not intercepted. Disabling shortcuts uses the formatted view in the same pane. Finishing focuses the results heading with the score. The dialog uses native modal behavior and restores its trigger’s focus.
