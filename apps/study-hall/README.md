...
- Shortcuts do not run in typing fields, editable areas, or dialogs. Buttons and links keep native Space/Enter activation. Typing fields keep their own Ctrl+Z. Other modifier combinations, composition, and key repeats are left alone.
- With shortcuts enabled, a small, named `role="application"` Keyboard study area receives focus. The same DOM element retains focus through reveal, rating, Again, and undo. A persistent polite, atomic live region announces the current side and card number; it includes image alternatives. Library, card content, settings, and results remain outside application mode. This follows the [WAI-ARIA application role guidance](https://www.w3.org/TR/wai-aria-1.2/#application).
- Escape or Read card pauses shortcuts and focuses the ordinary question group for reading rich content. Resume keyboard study returns to the keyboard area and announces the current side. Repeat current question or answer repeats that announcement. Tab and Shift+Tab are not intercepted. Disabling shortcuts removes application mode and retains the original question/answer focus behavior. Finishing focuses the results heading with the score. The dialog uses native modal behavior and restores its trigger’s focus.
