...
- With shortcuts enabled, one textarea inside a small, named `role="application"` Keyboard study area retains focus through reveal, rating, Again, and undo. Each change selects the current side's text. Its label identifies the side and card number; its value preserves paragraphs, lists, table rows, and image alternatives. Native arrows, Home/End, Shift+arrows, select-all, and copy work. A separate full-text live announcement is no longer emitted, avoiding duplicate speech alongside native text selection. Library, formatted card content, settings, and results remain outside application mode. This follows the [WAI-ARIA application role guidance](https://www.w3.org/TR/wai-aria-1.2/#application).
...
- The reader's caret and selection highlight are visually transparent. Native selection still exposes the current text and reading position to assistive technology and supports arrow navigation and copying; this does not directly set JAWS's virtual cursor. The visible focus outline remains.
- Escape or Read card pauses shortcuts and focuses the current question or answer group for reading rich content. Resume keyboard study and Repeat current question or answer focus the reader and select its text again. Tab and Shift+Tab are not intercepted. Disabling shortcuts removes the reader and application mode and retains the original question/answer focus behavior. Finishing focuses the results heading with the score. The dialog uses native modal behavior and restores its trigger’s focus.
